...
# # ======================== CONFIGURATION ========================
# INPUT_PATH = "datasets/processed/huawei_nokia_scenario_1.csv"
# MODEL_OUTPUT_PATH = "components/healthscore_training/models/huawei_nokia_healthscore.pkl"
# DATA_SPLIT_OUTPUT_PATH = "components/healthscore_training/models/huawei_nokia_data_split.pkl"
...
#     """Split dataset en train/test avec stratification"""
...
# def save_data_split(X_train, X_test, y_train, y_test, path):
...
#     save_data_split(X_train, X_test, y_train, y_test, DATA_SPLIT_OUTPUT_PATH)
...
TEST_SIZE          = 0.2
N_ESTIMATORS       = 100
MAX_DEPTH          = 7    # ← réduit (était 10) pour éviter overfitting
MIN_SAMPLES_SPLIT  = 10   # ← augmenté (était 5) pour meilleure généralisation
...
def split_features_target(df):
    """Sépare features, target et device_id"""
    print("\n🎯 Splitting features and target...")

    # ← CHANGEMENT : extraire device_id séparément
...
def split_train_test(X, y, device_ids, test_size=TEST_SIZE, random_state=RANDOM_STATE):
    ...
    # ← CHANGEMENT : inclure device_ids dans le split
...
def train_random_forest(X_train, y_train):
    ...
    print(f"  - max_depth:         {MAX_DEPTH}")
    print(f"  - min_samples_split: {MIN_SAMPLES_SPLIT}")
    print(f"  - class_weight:      balanced")   # ← CHANGEMENT
    print(f"  - random_state:      {RANDOM_STATE}")
    ...
    model = RandomForestClassifier(
        ...
        min_samples_split=MIN_SAMPLES_SPLIT,
        random_state=RANDOM_STATE,
        class_weight='balanced',   # ← CHANGEMENT : gère le déséquilibre des classes
        n_jobs=-1,
        verbose=1
    )
...
def save_data_split(X_train, X_test, y_train, y_test, device_id_train, device_id_test, path):
    ...
    data_split = {
        ...
        'y_train':          y_train,
        'y_test':           y_test,
        'device_id_train':  device_id_train,   # ← CHANGEMENT : vrais device_id
        'device_id_test':   device_id_test      # ← CHANGEMENT : vrais device_id
    }
...
def main():
    ...
    # 6. Sauvegarder les données de split (avec device_id)
