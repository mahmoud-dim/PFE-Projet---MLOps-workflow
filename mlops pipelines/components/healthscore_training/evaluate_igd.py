...
# ============================================================
# evaluate_igd.py
# Evaluation + Gatekeeping — Scénario 1 IGD
# Input  : MinIO → models/IGD_*.pkl / *.json
# Output : MinIO → results/scenario1/IGD_*.json / *.png
# ============================================================
...
REPORT_KEY      = "scenario1/IGD_classification_report.txt"
OPERATIONAL_KEY = "scenario1/IGD_operational_predictions.json"
CM_KEY          = "scenario1/IGD_confusion_matrix.png"
...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    print(f"📉 Overfit Gap    : {overfitting_gap:.4f} — {'✅ Good' if overfitting_gap < 0.10 else '⚠️ Moderate' if overfitting_gap < 0.15 else '❌ High'}")
...
def check_gatekeeping(metrics):
    print("\n" + "="*75)
    print("🚪 GATEKEEPING - Model Validation IGD")
    print("="*75)
    ...
    for i, (proba, pred_class) in enumerate(zip(probabilities, predictions)):
        ...
        results.append({
            ...
            "prediction": {
                ...
                "class_probabilities": {
                    "optimal": round(p_opt,3), "degraded": round(p_deg,3), "critical": round(p_crit,3)
                },
...
def save_confusion_matrix_to_minio(cm):
    ...
    ax.set_title('Confusion Matrix - IGD Health Score', fontsize=14, fontweight='bold')
...
def main():
    print("="*75)
    print("📊 EVALUATION - Scenario 1 : IGD")
    print("="*75)
