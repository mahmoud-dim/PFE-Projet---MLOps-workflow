...
# # ======================== CONFIGURATION ========================
# MODEL_PATH = "components/healthscore_training/models/IGD_healthscore.pkl"
# DATA_SPLIT_PATH = "components/healthscore_training/models/IGD_data_split.pkl"
...
# METRICS_OUTPUT_PATH = f"{EVALUATION_DIR}/IGD_metrics.json"
# CONFUSION_MATRIX_PATH = f"{EVALUATION_DIR}/IGD_confusion_matrix.png"
# CLASSIFICATION_REPORT_PATH = f"{EVALUATION_DIR}/IGD_classification_report.txt"
# FEATURE_IMPORTANCE_PATH = f"{EVALUATION_DIR}/IGD_feature_importance.png"
# OPERATIONAL_OUTPUT_PATH = f"{EVALUATION_DIR}/IGD_operational_predictions.json"
...
#     feature_labels = {
#         'downstream_curr_rate_kbps': 'Current Rate (Kpbs)',
#         'downstream_max_rate_kbps': 'Max Rate (Kpbs)',
#         'snr_margin_down_db': 'SNRMargin (db)',
#         'attenuation_down_db': 'Attenuation (db)',
#         'crc_errors_total' : 'Total Errors'
#     }

#     labels = [feature_labels.get(f, f) for f in features]
#     colors = ['#2E86AB', '#A23B72', '#F18F01', '#C73E1D','#4DA768']
...
# def compute_decision_metrics(model, X, device_prefix="box"):
#     """
#     Génère les métriques opérationnelles type opérateur :
#     confidence, health_score, risk_score, risk_level,
#     recommended_action, priority_level, auto_ticket
#     """
...
#         # Risk Score
#         risk_score = float(1 - health_score)
...
#         results.append({
#             "device_id": f"{device_prefix}_{i+1}",
#             "predicted_class": int(pred_class),
#             "class_probabilities": {
#                 "optimal": float(p_optimal),
#                 "degraded": float(p_degraded),
#                 "critical": float(p_critical)
#             },
...
#     print("✅ Operational scoring generated")
#     return results
...
#     operational_results = compute_decision_metrics(model, X_test)
...
#     print(f"🏭 Operational predictions saved to {OPERATIONAL_OUTPUT_PATH}")
...
#     else:
#         print(f"\n💡 Next Steps:")
#         print(f"  1. Tune hyperparameters in train_igd.py")
...
def load_data_split(path):
    ...
    y_train = data_split['y_train']
    y_test = data_split['y_test']
    device_id_train = data_split['device_id_train']
    device_id_test = data_split['device_id_test']
...
def plot_feature_importance(train_info, output_path):
    ...
    feature_labels = {
        'downstream_curr_rate_kbps': 'Current Rate (Kbps)',
        'downstream_max_rate_kbps': 'Max Rate (Kbps)',
        'snr_margin_down_db': 'SNRMargin (dB)',
        'attenuation_down_db': 'Attenuation (dB)',
        'crc_errors_total': 'CRC Errors Total'
    }
...
def compute_decision_metrics(model, X, device_ids):
    """
    Génère les métriques opérationnelles pour chaque device.

    Utilise les VRAIS device_id extraits du dataset (ex: IGD_001, IGD_042...)
    et non des indices générés artificiellement.

    Args:
        model      : modèle RandomForest entraîné
        X          : features du test set (DataFrame)
        device_ids : Series pandas contenant les vrais device_id
                     alignés avec X (même index)
    """
    print("\n🏭 Generating operational scoring...")

    class_map = {0: 'optimal', 1: 'degraded', 2: 'critical'}

    probabilities = model.predict_proba(X)   # shape: (n_samples, 3)
    predictions   = model.predict(X)         # shape: (n_samples,)

    # Convertir en liste pour accès positionnel
    ...
    for i, (proba, pred_class) in enumerate(zip(probabilities, predictions)):
        ...
        # ── VRAI device_id du dataset ────────────────────────────────────
        device_id = str(device_ids_list[i])   # ex: "IGD_042", "box_88"
        # ────────────────────────────────────────────────────────────────
...
def main():
    ...
    # 4. Évaluer le modèle (métriques ML classiques)
    metrics, cm, report = evaluate_model(model, X_train, X_test, y_train, y_test)
    ...
    # 9. ── OPERATIONAL OUTPUT avec vrais device_id ───────────────────────
    print("\n" + "="*75)
    ...
    operational_results = compute_decision_metrics(
        model=model,
        X=X_test,
        device_ids=device_id_test   # ← vrais device_id extraits du dataset
    )
