...
Basé sur lois physiques Shannon et caractéristiques DSL
VERSION : SNR/Attenuation = DRIVERS, CRC = CONSÉQUENCE
...
# ======================== CONFIGURATION ========================
OUTPUT_PATH = "dataset/datasets/igd_realistic.csv"
...
def calculate_theoretical_rate(snr_db, attenuation_db, max_rate):
    ...
    """
    snr_linear         = 10 ** (snr_db / 10)
    capacity_factor    = np.log2(1 + snr_linear) / 10
    attenuation_factor = max(0.1, 1 - (attenuation_db / 80))
    theoretical_rate   = max_rate * capacity_factor * attenuation_factor
    return min(theoretical_rate, max_rate)
...
def calculate_crc_errors_from_snr(snr_db, add_variance=True):
    """
    Calcule CRC errors basé UNIQUEMENT sur SNR
    CRC = CONSÉQUENCE de SNR, pas INPUT pour health_score
    Grosse variance pour éviter corrélation parfaite
    """
    if snr_db < 4:
        base_errors = np.random.uniform(1500, 2500)
    elif snr_db < 6:
...
def generate_device_timeseries(device_id, n_samples):
    """
    Génère série temporelle pour un device ADSL/VDSL.
    SNR + Attenuation = DRIVERS PRINCIPAUX
    Health_score basé UNIQUEMENT sur SNR/Attenuation
    CRC calculé APRÈS comme conséquence de SNR

    Chaque device a :
      - un profil fixe (stable, dégradation, interférence...)
      - des caractéristiques de ligne fixes (distance, max_rate)
      - 10 mesures espacées de 6h → couvre 2.5 jours
    """
    ...
    # Caractéristiques de ligne (fixes pour ce device)
    line_distance_km = np.random.uniform(0.5, 4.5)
    base_attenuation = 10 + (line_distance_km * 8)  # ~10-46 dB

    # Max rate dépend de la distance de ligne
    if base_attenuation < 25:
        max_rate_down = int(np.random.uniform(18000, 24000))
    elif base_attenuation < 40:
        max_rate_down = int(np.random.uniform(12000, 18000))
    ...
    for i in range(n_samples):
        ...
        if profile == 'stable_optimal':
            ...
            # 2. ATTÉNUATION (DRIVER #2)
            atten_down = base_attenuation + np.random.uniform(-2, 3)
            atten_up   = atten_down + np.random.uniform(0, 5)

            # 3. DÉBIT (fonction de SNR/Atten)
            theoretical_rate = calculate_theoretical_rate(snr_down, atten_down, max_rate_down)
            curr_rate_down = int(theoretical_rate * np.random.uniform(0.88, 0.98))
            curr_rate_up   = int(max_rate_up * np.random.uniform(0.85, 0.95))

            # 4. HEALTH basé sur SNR/Atten SEULEMENT
            if snr_down >= 15 and atten_down < 35:
                health_score = 0
            elif snr_down >= 10 and atten_down < 45:
                health_score = 0
            ...
            # 5. CRC EN DERNIER (conséquence de SNR)
            crc_errors = calculate_crc_errors_from_snr(snr_down, add_variance=True)

        # ========== PROFIL 2: LIGNE DÉGRADÉE (20%) ==========
        elif profile == 'line_degradation':

            # Progression temporelle : SNR se dégrade sur les 10 mesures
            progress = i / (n_samples - 1)  # 0.0 → 1.0

            # 1. SNR se dégrade (DRIVER) : 20 dB → 6 dB
            snr_down = 20 - (progress * 14)
            snr_up   = 17 - (progress * 12)
            ...
            # 4. HEALTH basé sur SNR/Atten UNIQUEMENT
            if snr_down < 6 or atten_down > 55:
                health_score = 2
            elif snr_down < 10 or atten_down > 45:
                health_score = 1
        ...
        # ========== PROFIL 4: PROBLÈME CONFIG (12%) ==========
            ...
            # 3. HEALTH basé sur ratio débit
            efficiency = curr_rate_down / theoretical_rate
            health_score = 1 if efficiency < 0.50 else 0
            root_cause = "Configuration / Ports overload"

            # 4. CRC modéré + erreurs de config
            base_crc      = calculate_crc_errors_from_snr(snr_down, add_variance=False)
            config_errors = int(np.random.uniform(100, 400))
            crc_errors    = base_crc + config_errors
        ...
        # ========== PROFIL 5: CROSSTALK / SATURATION (8%) ==========
            ...
            # Heures de pointe → crosstalk des voisins aggrave le SNR
            if 18 <= hour <= 23:
                snr_down -= np.random.uniform(2, 5)
                snr_up   -= np.random.uniform(2, 4)
        ...
        # Bruit réaliste sur les mesures (jamais parfaites)
        snr_down   = max(0, snr_down   + np.random.normal(0, 1.2))
        snr_up     = max(0, snr_up     + np.random.normal(0, 1.0))
        atten_down += np.random.normal(0, 1.5)
        atten_up   += np.random.normal(0, 1.5)

        # Variance finale sur CRC (bruit de mesure)
        crc_errors = int(crc_errors * np.random.uniform(0.8, 1.2))
        crc_errors = max(0, crc_errors)
...
def main():
    ...
    # Shuffle (mélange les lignes pour éviter tout biais d'ordre)
    df = df.sample(frac=1, random_state=42).reset_index(drop=True)
    ...
    for cause, count in df['root_cause'].value_counts().items():
        pct = count / len(df) * 100
        print(f"   {cause:35s}: {count:4d} ({pct:.1f}%)")

    print(f"\n📊 Line Quality Statistics:")
    print(f"   SNR Down   → mean: {df['snr_margin_down_db'].mean():.2f} dB"
          f"  | min: {df['snr_margin_down_db'].min():.2f}"
          f"  | max: {df['snr_margin_down_db'].max():.2f}")
    print(f"   Atten Down → mean: {df['attenuation_down_db'].mean():.2f} dB"
          f"  | min: {df['attenuation_down_db'].min():.2f}"
          f"  | max: {df['attenuation_down_db'].max():.2f}")
    print(f"   CRC Errors → mean: {df['crc_errors_total'].mean():.0f}"
          f"  | min: {df['crc_errors_total'].min()}"
          f"  | max: {df['crc_errors_total'].max()}")

    print(f"\n🔍 Feature Correlations with Health Score:")
    corr_cols = ['snr_margin_down_db', 'attenuation_down_db',
                 'crc_errors_total', 'health_score']
    corr = (df[corr_cols].corr()['health_score']
            .drop('health_score')
    ...
    print(f"   ✅ {N_DEVICES} devices uniques")
    print(f"   ✅ {N_SAMPLES_PER_DEVICE} snapshots temporels par device (série temporelle)")
    print(f"   ✅ SNR = driver principal")
    print(f"   ✅ CRC = conséquence du SNR")
    print(f"\n💡 Next steps:")
    print(f"   Scénario 1 → preprocess_igd_s1.py  (garde le plus récent → 300 lignes)")
    print(f"   Scénario 3 → preprocess_igd_s3.py  (garde tout → 3000 lignes time series)")
